...
OBS_ELEMENT_INDICES = {
    ...
    "panda_reciever_place": np.array([42, 43, 44]),
...
OBS_ELEMENT_GOALS = {
    ...
    "panda_reciever_wait": np.array([0.65, 0.15, 1.87]),
    ...
    "panda_reciever_place": np.array([0.5, -0.3, 0.785]),
...
class HandoverEnv(gym.Env, EzPickle):
    ...
    * `achieved_goal`: this key represents the current state of the tasks. The value is another `Dict` space with keys the tasks to be completed in the episode and values the
    ...
    def calculate_reward(
        ...
        # penalty for reciever robot not using fingers in pickup task
        ...
        combined_reward -= 0.005 * reviever_velocity_diff 

        # penalize changes in position to help with smoother movements
        reviever_position_diff = np.sum(np.abs(reciever_current_pos - reciever_prev_pos))
        combined_reward -= 0.005 * reviever_position_diff

         # pickup reward
        if "object_move_p1" in self.tasks_to_complete:
            
            combined_reward += self.pickup_reward(achieved_goal, desired_goal, bad_collisons, good_collisons, combined_reward, max_object_height, reciever_current_vel)

        # handover reward
        if "object_move_p2" in self.tasks_to_complete:

            combined_reward += self.handover_reward(achieved_goal, desired_goal, good_collisons, combined_reward, reciever_current_vel, giver_position_diff)         
    ...
    def step(self, action):
        ...
        reward = self.calculate_reward(self.goal, self.achieved_goal, robot_obs, self.max_object_height, collsions)
